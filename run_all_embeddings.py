...
# missing: 
# Config: layer_size=, model_name=Alibaba-NLP/gte-base-en-v1.5, use_narrations=False
...
# for use_narrations in [True]:
    for layer_size in layer_sizes:
        for model_name in model_names:
            ...
            # if 'gte-large' in model_name and use_narrations and len(layer_sizes) > 0:
            ...
            if 'gte-base' in model_name and use_narrations:
                ...
                sbatch_options += ["--partition=g80", "--gres=gpu:1"]
            ...
            for run in wandb.Api().runs("rug-minds/parse-ego4d"):
                layer_cond = ",".join(map(str, run.config.get("layer_sizes", [None]))) == layer_size
                ...
                filter_cond = run.config.get("min_correct", -1) == min_correct
                run_exists = all([layer_cond, name_cond, state_cond, narr_cond, filter_cond])
                if run_exists:
                    break
            ...
            if run_exists:
                ...
                subprocess.run([
                    ...
                    f"--min_correct={min_correct}",
                    f"--min_sensible={min_sensible}",
